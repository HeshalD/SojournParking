...
const AuthLoginRegMern = () => {
  ...
  const [errorMern, setErrorMern] = useState(""); // Define error state here
...
const LoginFormMern = ({ toggleFormMern, setErrorMern, errorMern }) => {
  ...
  const handleLoginMern = async (e) => {
    ...
    try {
      ...
      if (res.data.requires2FA) {
        ...
        localStorage.setItem("token-mern", res.data.token);
  ...
  const handle2FASubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5001/api/users/verify2FA", {
        tempToken: twoFAToken,
        code: twoFACode,
      });

      // Save token after 2FA verification
      localStorage.setItem("token-mern", res.data.token);
  ...
  return (
    <form onSubmit={handleLoginMern} className="auth-mern-glass-card">
      ...
      <p className="auth-mern-text-center auth-mern-mt-2">
        <a href="/forgot-password" className="auth-mern-btn-link">Forgot Password?</a>
      </p>
    </form>
  );
};
...
const LoginFormMern = ({ toggleFormMern, setErrorMern, errorMern }) => {
  ...
  const handleLoginMern = async (e) => {
    ...
    try {
      ...
      if (res.data.requires2FA) {
        ...
        // Set remember me flag if user chooses it
        if (rememberMe) {
  ...
  // Check if user has already chosen "Remember Me"
  useEffect(() => {
    const rememberMeFlag = localStorage.getItem("rememberMe");
    if (rememberMeFlag === "true") {
      // Skip 2FA if "Remember Me" is set
      window.location.href = "/userDashboard";
    }
  }, []);
  ...
  return (
    <form onSubmit={handleLoginMern} className="auth-mern-glass-card">
      ...
      <div className="auth-mern-remember-me">
        <input
          type="checkbox"
          checked={rememberMe}
