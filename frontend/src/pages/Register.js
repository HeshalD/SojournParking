...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:8070/user/register", submitData); // Update API endpoint to /user/register
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { confirmPassword, ...submitData } = formData; // exclude confirmPassword before sending to backend
      await axios.post("http://localhost:8070/user/register", submitData); // API endpoint for user registration
      alert("Registered Successfully");
      navigate("/login");
    } catch (err) {
      alert(err.response?.data?.message || "Registration Failed");
